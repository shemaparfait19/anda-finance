    ...
    <div className="flex justify-between items-center py-2">
      ...
      {isBadge ? (
        ...
        <span className="font-medium">
          {typeof value === "number" ? `RWF ${value.toLocaleString()}` : value}
        </span>
      )}
    </div>
  );
...
export default function ViewLoanDetailsDialog({
  loan,
  open,
  onOpenChange,
}: ViewLoanDetailsDialogProps) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          <DetailRow
            ...
            value={loan.status}
            isBadge
            badgeVariant={getStatusBadgeVariant(loan.status)}
          />
          <Separator />
          ...
          <DetailRow
            label="Issue Date"
            value={new Date(loan.issueDate).toLocaleDateString()}
          />
          <DetailRow
            label="Due Date"
