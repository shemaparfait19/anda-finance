...
export default function SavingsAccountsTable({ accounts, members }: SavingsAccountsTableProps) {
    ...
    const findMemberForAccount = (memberId: string) => members.find(m => m.id === memberId);

    const handleOpenDialog = (type: 'deposit' | 'withdrawal' | 'statement', account: SavingsAccount) => {
    ...
    return (
        <>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onSelect={() => handleOpenDialog('statement', account)}>View Statement</DropdownMenuItem>
                          <DropdownMenuItem onSelect={() => handleOpenDialog('deposit', account)}>Make Deposit</DropdownMenuItem>
                           <DropdownMenuItem onSelect={() => handleOpenDialog('withdrawal', account)}>Make Withdrawal</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
            ...
            {dialogState.deposit.account && (
                <NewDepositDialog
                    members={members.filter(m => m.id === dialogState.deposit.account?.memberId)}
             ...
             {dialogState.withdrawal.account && (
                <NewWithdrawalDialog
                    members={members.filter(m => m.id === dialogState.withdrawal.account?.memberId)}
