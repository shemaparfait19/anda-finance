...
import { loadInternalAccount } from '@/app/(app)/accounting/actions';
...
export default function LoadMirrorAccountDialog() {
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button size="sm" variant="outline" className="h-8 gap-1">
          ...
          <span className="sr-only sm:not-sr-only sm:whitespace-nowrap">Load Mirror Account</span>
        </Button>
      </DialogTrigger>
      ...
      <DialogContent className="sm:max-w-[440px]">
        <form ref={formRef} action={formAction}>
          <DialogHeader>
            <DialogTitle>Load Mirror Account</DialogTitle>
            <DialogDescription>
              Record incoming funds to a mirror / internal account.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            {/* Account Number */}
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="mirror-acct-no" className="text-right text-xs leading-tight">
                Account<br />Number
              </Label>
              <div className="col-span-3">
                <Input
                  id="mirror-acct-no"
                  name="accountNumber"
                  placeholder="Mirror / internal account number"
                  autoComplete="off"
                />
                {state.fields?.accountNumber && (
                  <p className="text-sm text-destructive mt-1">{state.fields.accountNumber}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="mirror-amount" className="text-right text-sm">Amount</Label>
              <div className="col-span-3 flex gap-2">
                <div className="flex-1">
                  <Input id="mirror-amount" name="amount" type="number" placeholder="RWF 0" />
                  {state.fields?.amount && (
